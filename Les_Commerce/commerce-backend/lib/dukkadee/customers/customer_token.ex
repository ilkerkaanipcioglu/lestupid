defmodule Dukkadee.Customers.CustomerToken do
  use Ecto.Schema
  import Ecto.Query
  alias Dukkadee.Customers.CustomerToken
  ...
  the token in the application to gain access. Furthermore, if the customer
  changes their email in the system, the tokens sent to the previous email are
  no longer valid.
  ...
  the starting point by definition for `verify_change_email_token_query/2`.
  """
  def verify_change_email_token_query(token, "change:" <> coming_email) do
    case Base.url_decode64(token, padding: false) do
      {:ok, decoded_token} ->
        hashed_token = :crypto.hash(@hash_algorithm, decoded_token)

        query =
          from token in token_and_context_query(hashed_token, "change:" <> coming_email),
            join: customer in assoc(token, :customer),
  ...
  """
  def customer_and_contexts_query(customer, contexts) do
