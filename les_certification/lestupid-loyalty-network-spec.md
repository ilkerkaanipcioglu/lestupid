...
The initial Harezm product registry lives in `certification-registry.json`. The first new first-party product spec is `../les_wait/lestupid-waiting-app-spec.md`.

Identity rule: a person registers once in the LesTupid identity layer. Other
apps only create activation records for that shared identity.
...
- `Les_Commerce/diy-marketplace-elixir`: certify DIY products, kits, services, workshops, and checkout flows.
- `Les_Commerce/quick-commerce-elixir`: allow merchants and marketplace products to become `candidate`, `claimed`, or `certified`.
- `Les_poke`: certify city-memory quests, clear loyalty points, and opt-in live discovery.
- `les_match`: certify consent-first matchmaking, explainable recommendations, labeled paid placement, and no manipulative ranking.
