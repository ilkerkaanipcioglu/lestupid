...
| Les Core | `les-core` | Lightweight contract app folder. | Shared identity status, app activation, channel activation, check-in event, opportunity event, and standard event-envelope contract. | All LesTupid apps | Elixir contract package; Phoenix HTTP shell planned | Keep small and separable; do not turn it into a heavy central platform before OAuth/OIDC is truly needed. `https://github.com/ilkerkaanipcioglu/lestupid/tree/main/les_core` |
...
| Les Wait | `les-wait` | Product/spec folder. May later contain API, mobile/web surfaces, bot channels and venue dashboards. | Breathable waiting, queue, appointment, and service-state app for canteens, clinics, student affairs, venues, and services; supports QR/table/sign/phone/proximity/staff tablet user entry plus venue owner/staff queue creation. | Les Go, Les Poke, Les Match, Les Commerce, Les Certification, Les Block | Spec/prototype; future API channels SMS/WhatsApp/USSD/Web and venue dashboard | Can stay monorepo while spec-first; split when queue API/venue dashboard becomes active. `https://github.com/ilkerkaanipcioglu/lestupid/tree/main/les_wait` |
...
| Les Core | Lightweight contract package. | None yet; future HTTP contract API. | Elixir contract modules for identity, activation and events. | None. | Elixir. | Initial tested contract. |
...
```json
{
  ...
  "app_groups": {
    ...
    "policy_and_proof": ["les-core", "les-certification", "les_block_adapter"],
    "commerce": [
      "lescommerce-core",
    ...
    "tools_and_flows": ["e-any"],
    "service_flow": ["les-wait", "les-care"],
