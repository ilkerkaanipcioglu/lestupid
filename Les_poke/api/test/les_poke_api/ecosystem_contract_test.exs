defmodule LesPokeApi.EcosystemContractTest do
  ...
  test "ecosystem product id matches app manifest and certification registry" do
    ...
    assert registry_product["manifest"] == "Les_poke/lestupid.app.json"
  end

  test "manifest contract includes required activation and portability fields" do
    manifest = Ecosystem.manifest("http://localhost:4003")
    ...
    assert Enum.any?(
             manifest.well_known,
             &String.ends_with?(&1, "/.well-known/lestupid-app.json")
           )
  end
